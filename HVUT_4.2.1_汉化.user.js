...
function $zh(value) {
  ...
  if (normalized.startsWith('[C] Open equipment link')) {
    return `[C] 在弹窗中打开装备链接
[V] 在新标签页打开装备链接
[L] 显示链接代码
[K] 以 BBCode 格式显示链接代码
[DOUBLE CLICK] 打开装备链接`;
  ...
  if (normalized.startsWith('[DOUBLE TAP] Open equipment link')) {
    return `[DOUBLE TAP] 打开装备链接
[LONG PRESS] 打开装备链接`;
  }
...
Item World`;
  ...
  if (normalized.startsWith('Syntax\n{$name}')) {
    return `语法
{$name}       装备名称
{$namecode}   带颜色/加粗的装备名称
{$url}        装备链接
{$eid}        装备 ID
{$_eid}       带透明下划线的 $eid，用于排版
{$level}      装备等级
{$pab}        装备 PAB
{$tier}       潜能层级（IW 等级）
{$price}      “price”输入框的值
{$note}       “note”输入框的值
              - 若包含 '$featured'，装备代码会加入“Featured”区块
{$condition ? text_if_true}
              - 若 $condition 为有效值，输出 'text_if_true'，否则不输出
              例：{$price? @ $price}
              - 如果装备库存中的该装备填写了“price”，会输出类似 ' @ 10m'。
{$condition ? text_if_true : text_if_false}
              - 若 $condition 为有效值，输出 'text_if_true'，否则输出 'text_if_false'。
              例：{$level ? Lv.$level : Souldbound}
              - 如果装备有等级，会输出类似 'Lv.500'，否则输出 'Soulbound'。`;
  }
  if (normalized.startsWith('Syntax\nBASE MATCH')) {
    return `语法
...
- BASE MATCH 使用装备过滤器规则。
- 每个 SUB MATCH 彼此独立。
- 例：Willow Staff of Destruction : name=bold ; Demonic : prefix=red ; Tempestuous || Shocking : prefix=orange
[选项关键字]
options : name（全名）, quality, prefix, type, slot, suffix
values : bold, rainbow，或任意颜色，如 'red', '#f00'
- 例：Peerless : quality=rainbow, name=bold`;
  }
  if (normalized.startsWith('Syntax\n()')) {
    return `语法
()   : 分组
...
||   : OR（或）
!    : NOT（非）
$QUALITY+   : 装备品质是否等于或高于指定 QUALITY
$pab=xyz    : 装备是否拥有 PAB x、y、z
$prefix     : 装备是否有前缀
$level      : 数字，装备等级
例：Magnificent && Power && !Warding
例：$Exquisite+ && (Rapier || Shortsword) && Slaughter && $prefix && $pab=sd && $level<250`;
  }
  let text = value;
  text = text.replace(/^Error: invalid code\n\n/s, '错误：代码无效\n\n');
  text = text.replace(/^Error: price must be a number\n\n/s, '错误：价格必须是数字\n\n');
  text = text.replace(/^Would you like to buy (.+)\?$/s, '要购买 $1 吗？');
  text = text.replace(/^Accepting the attachments will deduct ([\d,]+) Credits from your account\.\nAre you sure\?$/s, '领取附件将从你的账户扣除 $1 Credits。\n确定吗？');
  text = text.replace(/^This message was returned from (.+)$/s, '此邮件由 $1 退回');
  text = text.replace(/^(\d+) monster\(s\) brought you (\d+) gift\(s\), (.+) credits$/s, '$1 只怪物带来了 $2 个礼物，估值 $3 Credits');
  text = text.replace(/^For (.+) days \/ Since (.+)$/s, '统计 $1 天 / 自 $2 起');
  text = text.replace(/^- Total: (.+) gift\(s\), (.+) credits$/s, '- 总计：$1 个礼物，估值 $2 Credits');
  text = text.replace(/^- Daily: (.+) gift\(s\), (.+) credits$/s, '- 每日：$1 个礼物，估值 $2 Credits');
  text = text.replace(/Currently playing in/g, '当前服务器：');
  text = text.replace(/Click to switch to/g, '点击切换到');
  text = text.replace(/Your inventory is almost full\./g, '你的装备库存快满了。');
  text = text.replace(/Please manage your equipment to increase available capacity\./g, '请整理装备以增加可用容量。');
  text = text.replace(/Total ([\d,]+)/g, '总计 $1');
  text = text.replace(/^Next: /, '下一等级：');
  text = text.replace(/^Level: /, '等级：');
  text = text.replace(/^Total: /, '总计：');
  text = text.replace(/^Inventory Capacity: /, '库存容量：');
  text = text.replace(/^Total Cost: /, '总花费：');
  text = text.replace(/^You have ([\d,]+) credits worth of trophies in the inventory\.$/s, '库存中的奖杯约值 $1 Credits。');
  text = text.replace(/Spent Credits/g, '已花费 Credits');
  text = text.replace(/Crystal Pack/g, '水晶包');
  text = text.replace(/Protected Equipment/g, '受保护装备');
  return text;
}
$zh.dict = {
  'HV Utils Settings': 'HV Utils 设置',
  'Random Encounter': '随机遭遇',
  'Use Random Encounter Notification.': '启用随机遭遇提醒。',
  'Enable Random Encounter Notification in the battle.': '战斗中启用随机遭遇提醒。',
  'Enable Random Encounter Notification while browsing the gallery.': '浏览画廊时启用随机遭遇提醒。',
  'When opening Random Encounter from the gallery, it goes to alt.hentaiverse.org.': '从画廊打开随机遭遇时使用 alt.hentaiverse.org。',
  'Play a beep sound when Random Encounter is ready.\nThe order of values is [volume], [frequency], [duration].\nSet it to 0 to disable.': '随机遭遇可用时播放提示音。\n数值顺序为 [音量]、[频率]、[持续时间]。\n设为 0 可禁用。',
  'Top Navigation Bar': '顶部导航栏',
  'Integrate top menus into one button.': '将顶部菜单整合为一个按钮。',
  'Set quick links in the top.\nEach value can be an item in the default menu, or can be defined manually.\nA format is "TITLE | LABEL | URL (| SERVER)?"\nSERVER is optional; It must be either \'persistent\' or \'isekai\' and the link will only be created on that server.\nIf [topMenuIntegration] above is disabled, set the number of items in the list to 8 or less.': '设置顶部快捷链接。\n每一行可以是默认菜单项目，也可以手动定义。\n格式为 "TITLE | LABEL | URL (| SERVER)?"\nSERVER 可选；必须为 \'persistent\' 或 \'isekai\'，链接只会在对应服务器创建。\n如果上方 [topMenuIntegration] 已禁用，请将列表项目数设为 8 个或更少。',
  'Confirm whether to use a stamina restorative item.': '使用体力恢复物品前确认。',
  'Disable the stamina restorative button when stamina is above the specified value.': '体力高于指定值时禁用体力恢复按钮。',
  'Warn when the stamina is below the specified value.': '体力低于指定值时警告。',
  'Bottom Bar': '底部栏',
  'Show the credits balance.': '显示 Credits 余额。',
  'Show the free space in the Equipment Inventory.': '显示装备库存剩余空间。',
  'Warn when the free space in the Equipment Inventory is below the specified value.': '装备库存剩余空间低于指定值时警告。',
  'Shows the training in progress and automatically start the next training up to the set level.': '显示进行中的训练，并自动开始下一个训练直到设定等级。',
  'Show the weapon and the armor which are currently in the lottery.': '显示当前彩票中的武器和护甲。',
  'Notify if the new equipment in the lottery qualifies.\n* $pab is not available.': '彩票出现符合条件的新装备时提醒。\n* $pab 不可用。',
  'Equipment': '装备',
  'Integrate all types of equipment into a list.': '将所有装备类型整合为一个列表。',
  'Sort and categorize the equipment list.': '排序并分类装备列表。',
  'Set the color of equipment by quality.': '按品质设置装备颜色。',
  'Show equipment\'s level.': '显示装备等级。',
  'Show equipment\'s pab.': '显示装备 PAB。',
  'Show equipment\'s charms in the pop-up': '在弹窗中显示装备的附魔/护符信息',
  'Hide equipment\'s drop info in the pop-up': '在弹窗中隐藏装备掉落信息',
  'Support keyboard and mouse actions when the mouse cursor is over the equipment.': '鼠标悬停在装备上时支持键盘和鼠标操作。',
  'Support touch actions on mobile': '在移动端支持触摸操作',
  'Set the format of the code for the forum.': '设置论坛用装备代码格式。',
  'Set the rules for codes that decorate the names of equipment.': '设置用于装饰装备名称的代码规则。',
  'Equipment Shop': '装备商店',
  'Confirm when selling or salvaging equipment.': '出售或分解装备时确认。',
  'Automatically protect filtered equipment.': '自动保护被过滤器匹配的装备。',
  'Since the Market has a 1% fee, calculate the value of the material as 99% of the price.': '由于市场有 1% 手续费，按价格的 99% 计算材料价值。',
  'Show valuable equipment together at the top of the list, and prevent them from being selected by the "Select All" button.': '将有价值装备集中显示在列表顶部，并防止它们被“全选”按钮选中。',
  'Keep valuable equipment in BAZAAR, then hide all other trash.': '在 BAZAAR 中保留有价值装备，并隐藏其他垃圾。',
  'Monster Lab': '怪物实验室',
  'Advanced MonsterLab features': '启用 Monster Lab 高级功能',
  'Set the default value for sorting the list.': '设置列表默认排序。',
  'Close pop-ups by default.': '默认关闭弹窗。',
  'The Shrine': '祭坛',
  'Hide items to prevent them from being accidentally offered to the Shrine.': '隐藏物品，防止误献祭到祭坛。',
  'Show the names of rewarded equipment of higher quality only.\n* $pab is not available.': '只显示较高品质奖励装备的名称。\n* $pab 不可用。',
  'MoogleMail': 'MoogleMail',
  'Advanced MoogleMail features': '启用 MoogleMail 高级功能',
  'Battle': '战斗',
  'Armory': '装备库',
  'Set the position of the pane.': '设置面板位置。',
  'Warn if the durability of each equipment is low.': '装备耐久偏低时警告。',
  'Show the amount of items in the inventory, and warn if each number is less than the specified value.\nYou can purchase that quantity from the Item Shop by clicking on the item name in the list.\nIf the item name starts with # (#1 : 0, #2 : 0, ...), a space is inserted into the list.': '显示库存中的物品数量，并在数量低于指定值时警告。\n点击列表中的物品名称可从 Item Shop 购买该数量。\n如果物品名称以 # 开头（#1 : 0, #2 : 0, ...），会在列表中插入空行。',
  'disable': '禁用',
  'always': '总是',
  'on battle pages only': '仅战斗页面',
  'default': '默认',
  'click checkbox automatically': '自动勾选确认框',
  'do not confirm': '不确认',
  'left': '左',
  'right': '右',
  'power level': '战力等级',
  ...
  'Completed': '已完成',
  'max': '满级',
  'Processing other requests...': '正在处理其他请求...',
  'The file has been saved.': '文件已保存。',
  'Your equipment inventory is full': '你的装备库存已满',
  'Sold': '已出售',
  'Salvaged': '已分解',
  'Set': '设置',
  'Cancel Planning': '取消计划',
  'Plan Training...': '计划训练...',
  'Spent Credits': '已花费 Credits',
  'Save Current Settings': '保存当前设置',
  'Enter the name of the settings': '输入设置名称',
  'Offering Results': '献祭结果',
  'The Shrine Log': '祭坛日志',
  'Reset Log': '重置日志',
  'Edit Filters': '编辑过滤器',
  ...
  'Hunger': '饥饿',
  'Reset': '重置',
  'Increase / Decrease Primary Attributes': '增减主属性',
  'Equalize Primary Attributes': '均衡主属性',
  'Increase / Decrease Elemental Mitigations': '增减元素抗性',
  ...
  'Warding': '结界',
  'Swiftness': '迅捷',
  'Increases the gift factor by 2.5%': '礼物系数提高 2.5%',
  'Increases monster health by 5%': '怪物生命提高 5%',
  'Increases monster damage by 2.5%': '怪物伤害提高 2.5%',
  'Increases monster accuracy by 5%': '怪物命中提高 5%',
  'Decreases effective target evade/block by 1%': '目标有效闪避/格挡降低 1%',
  'Decreases effective target parry/resist by 1%': '目标有效招架/抵抗降低 1%',
  'Increases monster parry by 0.5%': '怪物招架提高 0.5%',
  'Increases monster resist by 0.5%': '怪物抵抗提高 0.5%',
  'Increases monster evade by 0.5%': '怪物闪避提高 0.5%',
  'Increases monster physical mitigation by 1%': '怪物物理减伤提高 1%',
  'Increases monster magical mitigation by 1%': '怪物魔法减伤提高 1%',
  'Increases monster attack speed by 2.5%': '怪物攻击速度提高 2.5%',
  'EQUIPMENT': '装备',
  'ITEM INVENTORY': '物品库存',
  'REPAIR ALL': '全部修理',
  'REPAIR': '修理',
  'Everything is fully repaired.': '所有装备都已完全修理。',
  'Unusable until repaired': '修理前无法使用',
  'Replace Charms & Pouches': '更换 Charms & Pouches',
  'Needs repair': '需要修理',
  'SEND': '发送',
  'Edit List': '编辑列表',
  'ATTACH from TEXT': '从文本添加附件',
  'Available Formats': '可用格式',
  'CALC': '计算',
  'ATTACH': '添加附件',
  'RESET': '重置',
  'Use Default MoogleMail': '使用默认 MoogleMail',
  'Clear': '清空',
  'Clear}': '清空',
  'send': '发送',
  'Item': '物品',
  'Credits / Hath': 'Credits / Hath',
  'Multi-Send': '批量发送',
  'count': '数量',
  'price': '价格',
  'cod': 'CoD',
  ...
  'Export to JSON': '导出 JSON',
  'Import from JSON': '导入 JSON',
  ...
  'Edit Format': '编辑格式',
  'Select:': '选择：',
  'Purchase & Salvage': '购买并分解',
  'Bazaar Filters': 'Bazaar 过滤器',
  'Protect Filters': '保护过滤器',
  'Protected Equipment': '受保护装备',
  'Item Prices': '物品价格',
  'To use HVUT, You have to set a [Custom Font] first.': '要使用 HVUT，需要先设置 [Custom Font]。',
  'The purchase request list is empty.': '购买请求列表为空。',
  'You do not have enough credits.': 'Credits 不足。',
  'Insufficient number of items in the Item Shop.': '物品商店库存不足。',
  'An error has occurred.': '发生错误。',
  'Some of the items needed to repair equipment cannot be bought from the Item Shop.': '部分修理装备所需物品无法从物品商店购买。',
  'Not enough materials for repairs.\nWould you like to purchase materials from the Item Shop to repair your equipment?': '修理材料不足。\n是否从物品商店购买材料来修理装备？',
  'Failed to generate a new Random Encounter key': '生成新的随机遭遇 key 失败',
  'Are you sure that you wish to use a stamina restorative?': '确定要使用体力恢复物品吗？',
  'Are you sure that you wish to cancel the current training?': '确定要取消当前训练吗？',
  'Select the major class of the equipment.': '请选择装备的大类。',
  'The Shrine log in this browser will be deleted.\nAre you sure?': '将删除此浏览器中的祭坛日志。\n确定吗？',
  'The Monster Lab log in this browser will be deleted.\nAre you sure?': '将删除此浏览器中的怪物实验室日志。\n确定吗？',
  'Not enough Crystals or Chaos Tokens': '水晶或 Chaos Token 不足',
  'Are you sure that you wish to upgrade the selected monsters?': '确定要升级选中的怪物吗？',
  'Remove attached items.': '请先移除已添加的附件。',
  'Are you sure that you wish to attach the protected equipment?': '确定要附上受保护装备吗？',
  'Insufficient number of items': '物品数量不足',
  'No recipient': '没有收件人',
...
function $zh_attr(e, an, av) {
  ...
  if (['text', 'textContent', 'innerText', 'innerHTML', 'title', 'placeholder', 'alt'].includes(an)) {
    return $zh(av);
  }
  if (an === 'value' && e.tagName === 'INPUT' && ['button', 'submit', 'reset'].includes(e.type)) {
    return $zh(av);
  }
  return av;
}
const alert = (message) => window.alert($zh(message));
const confirm = (message) => window.confirm($zh(message));
const prompt = (message, defaultValue) => window.prompt($zh(message), defaultValue);
function $element(t,p,a,f) {
  let e;
  if(t){e=document.createElement(t);}else if(t===''){e=document.createTextNode($zh(a));a=null;}else{return document.createDocumentFragment();}
  if(a!==null&&a!==undefined){
    function ao(e,a){Object.entries(a).forEach(([an,av])=>{
      if(typeof av==='object'&&av!==null){
        let obj;if(an in e){obj=e[an];}else{e[an]={};obj=e[an];}
        Object.entries(av).forEach(([sn,sv])=>{obj[sn]=(['header','desc'].includes(sn)?$zh(sv):sv);});
      }else{
        av=$zh_attr(e,an,av);
        if(an==='style'){e.style.cssText=av;}else if(an in e){e[an]=av;}else{e.setAttribute(an,av);}
      }
    });}
    function as(e,a){const an={'#':'id','.':'className','!':'style','/':'innerHTML'}[a[0]];if(an){e[an]=(an==='innerHTML'?$zh(a.slice(1)):a.slice(1));}else if(a!==''){e.textContent=$zh(String(a));}}
    if(typeof a==='string'||typeof a==='number'){e.textContent=$zh(String(a));}else if(Array.isArray(a)){a.forEach((a)=>{if(typeof a==='string'||typeof a==='number'){as(e,String(a));}else if(typeof a==='object'){ao(e,a);}});}else if(typeof a==='object'){ao(e,a);}
  }
  if(f){if(typeof f==='function'){e.addEventListener('click',f);}else if(typeof f==='object'){Object.entries(f).forEach(([ft,fl])=>{e.addEventListener(ft,fl);});}}
...
function $input(o,p,a,f) {if(typeof o==='string'){o=[o];}const [t,v,l,n,s]=o;let ao;if(!a){a={};ao=a;}else if(Array.isArray(a)){ao={};a.push(ao);}else if(typeof a==='object'){ao=a;}if(t==='select'){const i=$element('select',p,a,f);if(v){v.forEach((v)=>{v=split2(v,':');if(!v[1]){v[1]=v[0];}$element('option',i,{value:v[0],text:$zh(v[1])});});}return i;}ao.type=t;if(v||typeof v==='number'){ao.value=(['button','submit','reset'].includes(t)?$zh(v):v);}if(l){const b=$element('label',p);const i=$element('input',b,a,f);if(n==='before'){b.prepend($zh(l),' ');}else{b.append(' ',$zh(l));}if(s){$element('span',b);b.classList.add('hvut-label');}return i;}else{const i=$element('input',p,a,f);return i;}}
...
const $config = {
  ...
  validate: function (o) {
    ...
    if (error) {
      ...
      o.node.error.innerHTML = '<h3>' + $zh('Validation Error') + '</h3>' + html;
...
const $re = {
  ...
  refresh: function () {
    ...
    if (remain > 0) {
      ...
      $re.button.textContent = $zh(!$re.json.clear ? 'Expired' : 'Ready') + ` [${$re.json.count}]`;
      if ($re.beep) {
        $re.beep = false;
  ...
  run: async function (engage) {
    if ($re.type === 'ba') {
      ...
      $re.stop();
      $re.button.textContent = $zh('Checking...');
  ...
  load: async function (engage) {
    $re.stop();
    $re.get();
    $re.button.textContent = $zh('Loading...');
...
_top.init = function () {
  ...
  if ($config.settings.topMenuIntegration) {
    _top.node.menu['MENU'] = $element('div', menu_div, [`/<span>${$zh('MENU')}</span>`]);
  } else {
    Object.values(_top.menu).forEach((m) => {
      const g = m.g || m.s;
      if (!_top.node.menu[g]) {
        _top.node.menu[g] = $element('div', menu_div, [`/<span>${$zh(g).toUpperCase()}</span>`]);
      }
    });
  }
  ...
  _top.node.persona = $element('div', _top.node.div, ['!width: 110px;', `/<span>${$zh('Persona')}</span>`]);
  if ($config.settings.reNotification) {
    $re.hv();
  }
  ...
  Object.values(_top.menu).forEach((m) => {
    ...
    if (!ul[g]) {
      if ($config.settings.topMenuIntegration) {
        ...
        $element('li', ul[g], [$zh(g), '.hvut-top-menu-s']);
      } else {
...
_top.stamina_create = async function () {
  ...
  if (items.length) {
    ...
    p.textContent = $zh('No restorative available');
  }
};
...
const $dfct = {
  ...
  set_button: function (doc) {
    ...
    if (!value) {
      $dfct.node.button.textContent = '(D: 错误)';
      return;
    }
...
_bottom.show_equip = async function () {
  ...
  _bottom.node.equip.textContent = $zh(`Inventory Capacity: ${usage} / ${capacity}`);
...
_bottom.tr = {
  ...
  clock: function () {
    ...
    if (remain > 0) {
      ...
      _bottom.tr.node.link.textContent = $zh('Loading...');
  ...
  load: async function (post) {
    ...
    if (!$id('train_outer', doc)) {
      _bottom.tr.node.link.textContent = $zh('Waiting...');
    ...
    if ($id('train_progress', doc)) {
      ...
      if (error) {
        ...
        if ($qs(`img[onclick*="training.start_training(${json.next_id})"]`, doc)) {
          ...
          json.error = $zh("Can't start Training");
          _bottom.tr.node.link.textContent = json.error;
          setTimeout(_bottom.tr.clock, 60000);
        }
      } else {
        _bottom.tr.node.link.textContent = $zh('Training completed!');
      }
    } else {
      _bottom.tr.node.link.textContent = $zh('Training completed!');
    }
...
_bottom.lt = {
  ...
  load: async function (ss) {
    ...
    if (!eqname) {
      _bottom.lt.node[ss].equip.textContent = $zh('Failed to load');
      return;
    }
...
if (_query.s === 'Character' && _query.ss === 'ab') {
  ...
  _ab.calc = {
    ...
    table: function () {
      ...
      _ab.calc.node.table.innerHTML = `<thead><tr><td>${$zh('Level')}</td><td>${$zh('Ability Points')}</td><td>${$zh('Ability Boost')}</td><td>${$zh('Abilities')}</td></tr></thead><tbody></tbody>`;
...
if (_query.s === 'Bazaar' && _query.ss === 'ss') {
    ...
    load: async function (iid, reward_type, reward_slot) {
      ...
      */
      ...
      if (item.type === 'Trophy') {
        ...
        _ss.node.results_equip.value = $zh(`Inventory Capacity: ${_ss.equip.total} / ${_ss.equip.capacity}`) + (_ss.equip.sold ? `, ${$zh('Sold')}: ${_ss.equip.sold}` : '') + (_ss.equip.salvaged ? `, ${$zh('Salvaged')}: ${_ss.equip.salvaged}` : '');
        if (_ss.equip.total >= _ss.equip.capacity) {
          if (!_ss.error) {
            _ss.error = $zh('Your equipment inventory is full');
            popup(_ss.error);
          }
        }
  ...
  _ss.calc_trophies = function () {
    ...
    _ss.node.trophies.value = $zh(`You have ${_ss.trophies_value.toLocaleString()} credits worth of trophies in the inventory.`);
  };
  ...
  _ss.load_inventory = function () {
    $ajax.fetch('?s=Bazaar&ss=am&screen=organize').then((html) => {
      ...
      _ss.node.results_equip.value = $zh(`Inventory Capacity: ${_ss.equip.usage} / ${_ss.equip.capacity}`);
    });
  };
...
if (_query.s === 'Bazaar' && _query.ss === 'ml' && $config.settings.monsterLab) {
  if (_query.create) {
    ...
    GM_addStyle(/*css*/`
      ...
      .hvut-ml-wins::after { content: '最后更新: ' attr(data-update); position: absolute; top: 2px; right: 615px; border: 1px solid var(--color-border-default); padding: 2px 4px; line-height: 16px; background-color: var(--color-bg-h1); visibility: hidden; }
    ...
    _ml.main = {
      ...
      onerror: function (index) {
        ...
        mob.status = -1;
        mob.node.wins.classList.add('hvut-ml-outdated');
        mob.node.wins.textContent = $zh('failed');
      },
      edit_price: function () {
    ...
    _ml.upgrade = {
      ...
      init: async function () {
        ...
        _ml.upgrade.ct.forEach((ct, i) => { $element('td', thead, [ct.text.slice(0, 3).toLowerCase(), { dataset: { action: 'upgrade', index: 'all', type: 'ct', item: i, desc: `${$zh(ct.text)} : ${$zh(ct.desc)}` } }]); });
      ...
      update: async function () {
        ...
        _ml.upgrade.node.button.disabled = true;
        _ml.upgrade.node.button.value = $zh('Updating... ...');
        if (_ml.upgrade.node.run) {
          _ml.upgrade.node.run.disabled = true;
          _ml.upgrade.node.run.value = $zh('Updating...');
        }
        ...
        async function update(mob) {
          ...
          _ml.parse(mob, doc);
          _ml.upgrade.node.button.value = `${$zh('Updating...')} (${done}/${total})`;
          if (_ml.upgrade.node.run) {
        ...
        $config.set('ml_log', _ml.log);
        _ml.upgrade.node.button.disabled = false;
        _ml.upgrade.node.button.value = $zh('Monster Upgrader');
        if (_ml.upgrade.node.run) {
          _ml.upgrade.node.run.value = $zh('Completed');
        }
      },
    ...
    _mm.write = {
      ...
      pack: function (e) {
        if (_mm.write.pack.current) {
          popup($zh('Processing other requests...'));
          return;
        }
          ...
          conn.os.openCursor().onsuccess = function (e) {
            ...
            if (cursor) {
              ...
              if (completed === 0) {
                ...
                link.click();
                if (_mm.db.node.export) {
                  _mm.db.node.export.value = $zh('Completed');
                }
                popup(`<p>${$zh('The file has been saved.')}</p><p style="font-weight: bold;">${download}</p>`);
              }
            }
          };
        });
        ...
        function db_import(text) {
          try {
            ...
            function complete() {
              ...
              if (completed === 0) {
                if (_mm.db.node.import) {
                  _mm.db.node.import.value = $zh('Completed');
                }
              }
            }
